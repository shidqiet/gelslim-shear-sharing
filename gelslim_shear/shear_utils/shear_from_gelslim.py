...
class ShearGenerator:
    def __init__(
        ...
        pool_kernel_size: int = 25,
        Farneback_params=(0.5, 3, 15, 3, 5, 1.2, 0),
        output_size=(
            13,
            18,
        ),  # output size of shear data, 13x18 is the default, in the form (height, width)
        auto_reset=True,
        ...
        for channel in channels:
            if channel not in accepted_channels:
                raise ValueError(
                    "channel must be one of u, v, div, curl, sol_u, sol_v, irr_u, irr_v, dudt, dvdt, du, dv"
                )

        # if any of channels contain sol or irr
        if any(["sol" in channel for channel in channels]) or any(
        ...
        self.shear_field_tensor = torch.zeros(
            (self.num_channels, self.output_size[0], self.output_size[1])
        )
    ...
    def extract_flow_field(self, ref, image):
        flow = cv2.calcOpticalFlowFarneback(
            ...
            image,
            None,
            self.Farneback_params[0],
            self.Farneback_params[1],
            self.Farneback_params[2],
            self.Farneback_params[3],
            self.Farneback_params[4],
            self.Farneback_params[5],
            self.Farneback_params[6],
        )
        flow_dwn = skimage.measure.block_reduce(
            flow, (self.pool_kernel_size, self.pool_kernel_size, 1), np.mean
        )
    ...
    def update_shear(self):
        image = self.get_grayscale(
            self.tactile_image.permute(1, 2, 0).numpy().astype(np.uint8)
        )
        if self.prev_tactile_image is None:
            self.prev_tactile_image = self.tactile_image
        prev_shear_image = self.get_grayscale(
            self.prev_tactile_image.permute(1, 2, 0).numpy().astype(np.uint8)
        )
        no_shear_image = self.get_grayscale(
            self.base_tactile_image.permute(1, 2, 0).numpy().astype(np.uint8)
        )
        if self.method == "weighted" or self.method == "1":
            udiff, vdiff = self.extract_flow_field(prev_shear_image, image)
            if (
                "u" in self.channels
                or "v" in self.channels
                or "div" in self.channels
                or "curl" in self.channels
                or "sol_u" in self.channels
                or "irr_u" in self.channels
                or "sol_v" in self.channels
                or "irr_v" in self.channels
            ):
        ...
        if self.method == "weighted" or self.method == "2":
            if self.method == "2":
                if (
                    "dudt" in self.channels
                    or "dvdt" in self.channels
                    or "du" in self.channels
                    or "dv" in self.channels
                ):
                    udiff, vdiff = self.extract_flow_field(prev_shear_image, image)
            if (
                "u" in self.channels
        ...
        if self.method == "weighted":
            if (
                ...
                weight = self.sigmoid_weighting_function(
                    self.displacement,
                    self.displacement_midpoint,
        ...
        if "dudt" in self.channels:
            ...
            self.shear_field_tensor[dudt_channel_index] = torch.tensor(
                udiff / (self.time - self.prev_time)
            )
        if "dvdt" in self.channels:
            dvdt_channel_index = self.channels.index("dvdt")
            self.shear_field_tensor[dvdt_channel_index] = torch.tensor(
                vdiff / (self.time - self.prev_time)
            )
