...
class ShearPlotter:
    def __init__(
        self,
        ch_dim=0,
        max_scalar_magnitude=6,
        scale=6,
        channels: List[str] = ["u", "v"],
        titles=None,
        colors: List = ["blue"],
        cmaps: List[str] = ["seismic"],
        num_fingers=1,
        base_figsize=(7, 5),
    ):
        self.ch_dim = ch_dim
        ...
        self.cmaps = cmaps
        self.scalar_fields = [
            channel for channel in self.channels if channel in ["div", "curl"]
        ]
        self.vector_fields = [
            channel for channel in self.channels if channel not in ["div", "curl"]
        ]
        self.non_scalar_fields = []
        [
            self.non_scalar_fields.append(channel.replace("u", "").replace("v", ""))
            for channel in self.vector_fields
            if channel.replace("u", "").replace("v", "") not in self.non_scalar_fields
        ]
        if len(self.non_scalar_fields) != len(self.vector_fields) // 2:
            raise ValueError(
                "Must include both u and v components for each vector field for plotting"
            )
        ...
        if self.num_scalar_fields != len(self.cmaps):
            if len(self.cmaps) == 1:
                ...
                raise ValueError(
                    "Number of colormaps must match number of scalar fields"
                )
        self.shear_field_tensors = []
        figsize = (base_figsize[0] * num_fingers, base_figsize[1] * len(self.channels))
        self.fig, self.ax = plt.subplots(
            len(self.scalar_fields) + len(self.non_scalar_fields),
            num_fingers,
            figsize=figsize,
        )
        if num_fingers == 1:
    ...
    def plot_shear_info(self, shear_field_tensors):
        ...
        for finger in range(self.num_fingers):
            ...
            for i, scalar_field in enumerate(self.scalar_fields):
                sf = get_channel(
                    shear_field_tensor, self.channels.index(scalar_field), self.ch_dim
                )
                plot = plot_scalar_field(
                    self.ax[i + len(self.non_scalar_fields), finger],
                    sf,
                    max_magnitude=self.max_scalar_magnitude,
                    cmap=self.cmaps[i],
                    title=self.titles[i + len(self.non_scalar_fields)],
                )
                self.plots.append(plot)
    ...
    def update_shear_info(self, frame, shear_field_tensors):
        ...
        for i, finger in enumerate(range(self.num_fingers)):
            ...
            for j, non_scalar_field in enumerate(self.non_scalar_fields):
                ...
                vf = get_channel(shear_field_tensor, [u_index, v_index], self.ch_dim)
                plot_index = (
                    i * (len(self.non_scalar_fields) + len(self.scalar_fields)) + j
                )
            ...
            for j, scalar_field in enumerate(self.scalar_fields):
                ...
                plot_index = (
                    i * (len(self.non_scalar_fields) + len(self.scalar_fields))
                    + j
                    + len(self.non_scalar_fields)
                )
